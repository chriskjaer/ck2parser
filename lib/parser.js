...
var parser = {
  ...
  convertToBooleans: replace(/\b(yes|no)(?=\n)/g, function(match) {
    return match === 'yes' ? true : false;
  }),

  //Paradox, why is this the only value not in quotes?
  wrapTempleInQuotes: replace(/temple(?=\n)/g, '"$&"'),

  minify: replace(/(\n|\t|\s)/g, ''),
};

parser.everything = R.pipe(
  parser.checkIfCK2SaveFile,
  parser.replaceFiletypeWithStartCurly,
  parser.wrapKeysInQuotes,
  parser.createArrays,
  parser.replaceEqualsWithColons,
  parser.convertToBooleans,
  parser.wrapTempleInQuotes,
